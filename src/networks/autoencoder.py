# https://github.com/databaaz/ResNetVAE/blob/master/modules.py
...
def convtrans2D_output_size(img_size, padding, kernel_size, stride):
    # compute output shape of conv2D
    outshape = (
        (img_size[0] - 1) * stride[0] - 2 * padding[0] + kernel_size[0],
...
class ResNet_Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        x = self.convTrans6(x)
        x = self.convTrans7(x)
        # x = self.convTrans8(x) # kick out since we want 8 channels
...
class ResNet_Encoder(nn.Module):
    def __init__(
        ...
        self.downsample = nn.Linear(input_channels, 3)
...
# Resources:
# https://www.hindawi.com/journals/mpe/2022/7088137/