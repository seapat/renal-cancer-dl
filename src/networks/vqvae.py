...
# reason: https://www.reddit.com/r/MachineLearning/comments/nxjqvb/d_preventing_index_collapse_in_vqvae/
class Quantize(nn.Module):
    def __init__(self, dim, n_embed, decay=0.99, eps=1e-5):
        super().__init__()

        self.dim: Tensor = dim
        self.n_embed: Tensor  = n_embed
        self.decay: float = decay
        self.eps: float = eps

        embed: torch.Tensor = torch.randn(dim, n_embed)
...
class Encoder(nn.Module):
    def __init__(self, in_channel, channel, n_res_block, n_res_channel, stride):
        ...
        if stride == 16:
            ...
            raise ValueError('stride must be 2, 4, 8, or 16')
...
#         self.model.classifier = nn.Linear(self.num_ftrs, n_classes)
#         self.act = nn.Sigmoid()
